...
This population distribution is right skewed, so the mean house area is greater than the median house area. There are definitely outliers here.
...
The distribution of the sample is similar to the distribution of the population: right-skewed with outliers.
...
The mean of samp2 is lower than the mean of samp1 and closer to the population mean. The larger the sample size, the better the approximation to our population mean.
...
There are 5000 mean values (elements) in 'sample_means50'. The sampling distribution appears to be normal and centered around the population mean. I don't expect the distribution to change much if we instead collected 50,000 sample means, but it would be an even better approximation of the true mean.
...
There are 100 elements in this object, and each element represents a mean of a sample of 50 house areas.
...
As the sample size is larger, the center is closer to the true population mean and there is less spread in the distribution of sample means, ie less variance.
...
Using this sample, by best point estimate of the population mean is 186,153.
    
```{r price}
sampr <- sample(price, 50)
summary(sampr)
```
...
This sampling distribution is slightly right-skewed but mostly normal. I would guess that the mean home price of the population is 180,000. The true population mean is 180,797.
...
There is less variance when the sample size is 150 versus 50. The distribution here also appears more normal. I would guess the mean sale price to be 180,000.
...
The distribution from 3 has a smaller spread. If we want estimates that are more often close to the true value, we prefer a distribution with a small spread (larger sample size).
