...
A scatter plot displays the relationship between runs and at_bats. The relationship looks linear, though many of the points are dispersed around the line. I would still feel comfortable using a linear model to predict the number of runs.
...
There is a positive linear relationship between these two variables. Based on the correlation, it is a relatively strong relationship, though there are several outliers in the plot.
...
The smallest sum of squares for me was 227029.3 from run 5. It is greater than the sum of squares that was calculated above.
...
The slope tells us that the number of runs tend to increase as the number of homeruns increase for a team.
...
The least squares regression line is an overestimate of runs when there are 5,578 at-bats. The residual is 14.6861.
...
There does not appear to be a pattern in the residuals of the linear regression, so the relationship between runs and at-bats is linear.
...
Based on the histogram and the normal probability plot, this condition appears to be met.
...
Yes, this condition also appears to be met.
...
At a glance, there does not see to be a linear relationship between stolen_bases and runs.

```{r wins}
reg <- lm(mlb11$runs ~ mlb11$stolen_bases)
...
The R$^2$ value for the model with at_bats is 123721.9 while it is 196706.3 for the model that uses stolen_bases to predict runs. My varaible does not predict runs better than at_bats because the sum of squares is greater.
...
bat_avg best predicts runs with an R$^2$ value of 67849.52.
...
With R$^2$ values of 12837.66, 20345.54, and 29768.7, these variables are generally more effective at predicting runs than the old variables. new_obs seems to be the best predictor of runs. 
...
The model diagnostics for the predictive model that uses new_obs as the explanatory variable all seem to check out.

```{r bestModel}
plot(mlb11$new_obs, mlb11$runs)
abline(lm(mlb11$runs~mlb11$new_obs))
hist(mlb11$new_obs)
qqnorm(lm(mlb11$runs~mlb11$new_obs)$residuals)
```
