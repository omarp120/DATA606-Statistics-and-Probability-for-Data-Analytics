...
The distribution of heights for men seems to be more normal and more unimodal than that of women. The heights of men also seem to range more than those of women.
...
Based on this plot, it appears that the data follow a nearly normal distribution.
...
Not all of the points fall on the line here either. Similar to the real data, the end points seem
to trail off and deviate from the line. There is, however, just a bit less deviation from the line
...
The normal probability for our data does look similar to the plots created for the simulated data, 
so there is evidence that female heights are nearly normal.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
Female weights do not appear to come from a normal distribution.
...
What is the probability that a female is shorter than the mean female height?

```{r femHgt}
pnorm(q = fhgtmean, mean = fhgtmean, sd = fhgtsd)
sum(fdims$hgt < fhgtmean) / length(fdims$hgt)
```

What is the probability that a female weighs less than the mean female weight?

```{r femWgt}
fwgtmean <- mean(fdims$wgt)
fwgtsd <- mean(fdims$wgt)
pnorm(q = fwgtmean, mean = fwgtmean, sd = fwgtsd)
sum(fdims$wgt < fwgtmean) / length(fdims$wgt)
```

Height had a closer agreement between the two methods than weight did. This makes sense because we determined through our exploration that height is more normal than weight.
...
This is the case because these distributions have many of the same values repeated, causing the stepwise pattern.
...
Based on this normal probability plot, this variable's distribution is right skewed. 
