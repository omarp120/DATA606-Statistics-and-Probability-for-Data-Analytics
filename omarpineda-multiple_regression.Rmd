...
Observational studies. We should phrase it as "does beauty influence student ratings"?

2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    that tell you about how students rate courses? Is this what you expected to 
    see? Why, or why not?

The distribution is left skewed as most students tend to rate their courses with higher scores. I thought most students would consider their teachers to be average but they are actually quite satisfied with them.
...
There appears to be no strong relationship between age and score received. Tenure track professors tend to be younger than teaching and tenured professors.
...
The initial plot was misleading because several points were layed on top of one another.
...
Equation: Y = 0.06664(bty_avg) + 3.88

Beauty score is a statistically significant predictor with a p-value = 5.08e-05
...
The residuals appear to be nealy normal and there doesn't seem to be a relationship among the residuals, so a linear model is a good fit.
...
Based on these diagnostic plots, the conditions seem to be satisfied.
...
Yes, it is still significant. Gender has changes the parameter estimate slightly.
...
Equation: Y =  3.75 + 0.1724 + 0.074(bty_avg)
...
R creates a dummy variable for categorical variables that have more than two levels.
...
Pic_color would be expected to have the highest p-value in this model.
...
cls_credits actually has the lowest p-value.
...
The score tends to be .123 higher when the professor is not a minority.
...
The coefficients and significance of the other explanatory variables changed slightly. The profs variable has minimal collinearity with the other explanatory variables.

```{r modelNew}
m_full_2 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval +  cls_students + cls_level + 
...
Equation: Y = 3.77+0.17(ethnicity_not_minority)+0.21(gendermale)−0.206(language_non_english)−0.006(age)+0.004(cls_perc_eval)+0.505(cls_credits_one_credit)+0.05(bty_avg)−0.19(pic_color_color)
...
This model satistifed the conditions.    
...
Professors at the university for longer will have more observations and courses taught by the same professor will have similar scores. The observations may not be entirely independent.

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.

Characteristics of a high evaluation scoring professor: not a minority, male, a native english speaker, young, have a high evaluation completion rate, teaches one credit score, rated to have high beauty with a black-and-white photo.

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?
    
I would be comfortable generalizing if the professors were similar to the general population. I would generalize to other universities with similar faculty (size, demographics, etc).
