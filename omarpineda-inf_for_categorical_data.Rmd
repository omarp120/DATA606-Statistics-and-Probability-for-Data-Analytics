...
We must assume that the sampling method was random and sufficient in size. Given the associated costs to do this for a sample that is representative of the world's population, this report may not be generalizable.
...
Each row of table 6 coresponds to a country. Each row of 'atheism' corresponds to an individual respondent in any given country.
...
The proportion of atheist reponses that we calculated is also 5%.
...
1. The sample observations are independent, which they are because each response is from an individual person. This is assuming that these respondents do not know each other, since individuals that know each other, for example family members, may be more likely to respond similarly.
2. We have at least 10 atheists and 10 non-atheists
...
I've chosen Japan and Brazil, and the conditions for inference are met by both subsets. The ME for Japan is 0.026 and the ME for Brazil is 0.004.

```{r twoCountries}
jp12 <- subset(atheism, nationality == "Japan" & year == "2012")
inference(jp12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
(0.3329-0.281)/2
bz12 <- subset(atheism, nationality == "Brazil" & year == "2012")
inference(bz12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
(0.0143-0.0056)/2
```
...
The relationship between p and me is parabolic. For 0 < p < 0.5, me increases as p increases. For 0.5 < p < 1.0, me decreases as p increases.
...
This sampling distribution of sample proportions has mean = 0.099 and sd = 0.009. It is centered at 0.10 and symmentrical.
...
These three new plots are centered at their p values and symmetrical. As n increases, the variance of the distribution decreases, ie the distribution is narrower. As p decreases, the variance decreases and the distribution becomes narrower. A higher n and lower p leads to a narrower distribution.

```{r getSampDist}
getDist <- function(n, p) {
...
p_hats400 <- getDist(400, 0.1)
p_hats1040 <- getDist(1040, 0.02)
p_hats400b <- getDist(400, 0.02)

par(mfrow = c(2, 2))
hist(p_hats, main = "p = 0.1, n = 1040", xlim = c(0, 0.18))
hist(p_hats400, main = "p = 0.1, n = 400", xlim = c(0, 0.18))
hist(p_hats1040, main = "p = 0.02, n = 1040", xlim = c(0, 0.18))
hist(p_hats400b, main = "p = 0.02, n = 400", xlim = c(0, 0.18))
par(mfrow = c(1, 1))
```
...
These distributions are shown above, and they appear normally distributed, so it is sensible to proceed with inference and report margin of errors as the report does.
...
1. Sample observations are independent within and between both groups
2. There are at least 10 atheists and 10 non-atheists within each year

The confidence interval for the true proportion of atheists in 2012 is (0.0734, 0.1065), and it is (0.083, 0.1177) in 2005. Since these intervals overlap, there is no significant difference in the true proportion of atheists in Spain between 2005 and 2012.
...
The confidence interval for the true proportion of atheists in 2012 is (0.0364, 0.0634), and it is (0.0038, 0.0161) in 2005. Since these intervals do not overlap, there is a significant difference in the true proportion of atheists in the U.S. between 2005 and 2012.
...
A Type 1 error occurs when we reject H0 in favor of Ha when H0 is in fact true. We expect this to happen, that is we expect to see a change when there is in fact is no change, in 5% of the countries.
...
The margin of error is maximized when p = 0.5, so we need need to find an n so that the margin of error is still less than 1% in this worst case scenario. We would have to sample 9,604 people to ensure that we are within the guidelines.

0.01 = 1.96 * sqrt((.5*.5)/n)
