...
`genhlth` - categorical - ordinal, `exerany` - categorical - nominal, `hlthplan` - categorical - nominal, `smoke100` - categorical - nominal, `height` - numerical - discrete, 
`weight` - numerical - continuous, `wtdesire` - numerical - continuous, `age` - numerical - discrete, and `gender` - categorical - nominal
...
There are 9569 males in the sample. 23.285% of the sample reports being in excellent health.
...
The mosaic plot reveals that a larger portion of males have smoked 100 cigarettes in their lifetime than females have.
...
This box plot shows that the average BMI of this sample is lower for those in excellent health, and
that health deteriorates as BMI increases. I chose to look at exerany and BMI as I'd expect those
who do not exercise to generally have a higher BMI. The figure seems to suggest that this is the case.
...
The histogram with more breaks has more detail that gives us a better idea on what the mode may be.
...
There is a positive linear relationship between weight and desired weight. As weight increases, the
desired weight also increases.
...
wdiff is an integer and a numerical data. If wdiff is 0, the person is at their desired weight. If
wdiff is positive, the person would like to gain weight. If wdiff is negative, the person would like
to lose weight.
...
wdiff is left-skewed with a center near 0 and outliers that are beyond -200 and 400. This
tells us that most people are generally happy with their weights and would only like to gain
...
On average, the men would like to lose 10.7 pounds while women in this sample would like to lose 18.2
pounds. The boxplot for men reveals that there is more variance in the difference between weight and desired weight than there is for women.

```{r wtOutlookGender}
msub <- subset(cdc, cdc$gender == "m")
fsub <- subset(cdc, cdc$gender == "f")
mean(msub$wdiff)
mean(fsub$wdiff)
...
The mean weight is 169.68 with a standard deviation of 40.08. The subset table of CDC, oneSD, only has observations that are within one standard deviation of the mean, and this table has 14,152 rows. Dividing this number by 20,000, we get that 70.76% of the weights in CDC are within one standard deviation of the mean.

```{r weightExplore}
mean(cdc$weight)
sd(cdc$weight)
upper <- mean(cdc$weight) + sd(cdc$weight)
lower <- mean(cdc$weight) - sd(cdc$weight)
oneSD <- subset(cdc, cdc$weight >= lower & cdc$weight <= upper)
head(oneSD)
dim(oneSD)
14152/20000
```
